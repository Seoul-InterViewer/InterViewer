...
export const questions = [
  ...
  useCallback은 함수를 **재사용** 해서 불필요한 함수 재생성을 방지해요.<br/>
  예를 들어, 자식 컴포넌트에 콜백 함수를 props로 넘길 떄, 매 렌더마다 함수가 새로 생성되면 자식도 불필요하게 **리렌더** 될 수 있어요. 그럴 땐 useCallback을 사용하는것이 효과적이죠.<br/> 
  개인적으로는 최적화가 실제로 필요할 때만 사용하는 편이에요. 지나치게 남발하면 오히려 **코드 복잡도**만 올라가고 
  성능 이점이 없을 수도 있거든요.`,
    difficulty: "medium",
    likes: 10,
    isSample: true,
